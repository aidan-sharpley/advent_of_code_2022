...
def part_one(data, priority_dict):
    ...
    result = sum([priority_dict[c] for c in common])
...
def main():
    with open("./inputs/" + INPUT_FILE, "r") as input_file:
        data = [
            [
                pair.strip("\n")[: int(len(pair) / 2)],
                pair.strip("\n")[int(len(pair) / 2) :],
        ...
        # Lowercase item types a through z have priorities 1 through 26.
        # Uppercase item types A through Z have priorities 27 through 52.
        priority_dict = {letters[x - 1]: x for x in range(1, 53, 1)}

        print(part_one(data, priority_dict))
        # print(part_two(data))4
