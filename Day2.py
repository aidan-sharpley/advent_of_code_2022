...
# the players each simultaneously choose one of Rock, Paper, or Scissors using a hand shape. Then, a winner for that round is selected: 
...
# Rock defeats Scissors, Scissors defeats Paper, and Paper defeats Rock.'
# a or x is rock, y or b is paper, c or z is scissor
# The score for a single round is the score for the shape you selected (1 for Rock, 2 for Paper, and 3 for Scissors)
# X means you need to lose, Y means you need to end the round in a draw, and Z means you need to win.
lkup = {
    # Rock
    'A':{'B':False, 'C': True, 'pt':1, 'outcome':-1},
    # Paper
    'B':{'A':True, 'C':False, 'pt':2, 'outcome':0},
    # Scissors
    'C':{'A':False, 'B':True, 'pt':3, 'outcome':1}
}
...
def rps(pair):
    ...
    my_play = pair[1]
    roshambo = lkup[my_play]

    # Points from my elven weapon of choice
    sum += roshambo['pt']
...
def part_two(d):
    ...
    for pair in d:
        opponent_move = pair[0]
        needed_outcome = lkup[pair[1]]['outcome']

        # Lose
        if needed_outcome < 0:
            for k,v in lkup[opponent_move].items():
                if len(k) > 1:
                    continue

                if v == True:
                    sum += rps([opponent_move, k])

        # Draw
        elif needed_outcome == 0:
            sum += rps([opponent_move, opponent_move])
